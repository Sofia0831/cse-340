...
async function getAccountInfo(account_id) {
  try {
    const result = await pool.query('SELECT account_id, account_firstname, account_lastname, account_email, account_password FROM account WHERE account_id = $1', [account_id])
      return result.rows[0]
  } catch (error) {
...
module.exports = {registerAccount, checkExistingEmail, getAccountByEmail, getAccountInfo, updateAccount, updatePassword}